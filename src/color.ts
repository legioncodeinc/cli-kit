 ...
 */
function paint(s: string, open: string, close: string): string {
	return enabled ? `${open}${s}${close}` : s;
}

/** Bold intensity. Identity when color is disabled. */
export const bold = (s: string): string => paint(s, sgr("1"), sgr("22"));

/** Dim/faint intensity. Identity when color is disabled. */
export const dim = (s: string): string => paint(s, sgr("2"), sgr("22"));

/** Red foreground. Identity when color is disabled. */
export const red = (s: string): string => paint(s, sgr("31"), sgr("39"));

/** Green foreground. Identity when color is disabled. */
export const green = (s: string): string => paint(s, sgr("32"), sgr("39"));

/** Yellow foreground. Identity when color is disabled. */
export const yellow = (s: string): string => paint(s, sgr("33"), sgr("39"));

/** Cyan foreground. Identity when color is disabled. */
export const cyan = (s: string): string => paint(s, sgr("36"), sgr("39"));

/** Honeycomb amber — the brand accent (256-color foreground `38;5;214`). Identity when color is disabled. */
export const amber = (s: string): string => paint(s, sgr("38;5;214"), sgr("39"));
