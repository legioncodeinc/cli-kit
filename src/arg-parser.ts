...
const success = (args: ParsedArgs): ParseResult => ({ ok: true, args });
/** Internal failure constructor. */
const failure = (error: string): ParseResult => ({ ok: false, error });

/** Outcome of applying a single flag token to the running flag map. */
interface ApplyOk {
  ok: true;
  /** Number of *additional* tokens consumed beyond the flag token itself (0 or 1, for the value). */
  advance: number;
}
type ApplyResult = ApplyOk | { ok: false; error: string };

/**
 * Build the human-readable constraint suffix for an int flag, e.g. `>= 1`,
 * `<= 100`, or `>= 1 and <= 100`. Empty string when neither bound is set.
 */
function intConstraint(spec: { min?: number; max?: number }): string {
  const hasMin = spec.min !== undefined;
  const hasMax = spec.max !== undefined;
  if (hasMin && hasMax) return `>= ${spec.min} and <= ${spec.max}`;
...
export function parseArgs(argv: string[], options?: ParseOptions): ParseResult {
  ...
  try {
    const list: readonly string[] = Array.isArray(argv) ? argv : [];

    // Index recognized flags by long name.
    const flagsByName = new Map<string, FlagSpec>();
    if (options?.flags) {
      for (const f of options.flags) {
        if (f && typeof f.name === "string") flagsByName.set(f.name, f);
      }
    }
