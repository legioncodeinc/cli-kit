...
/**
 * Write a single line to a stream, normalizing the trailing newline so callers
 * never produce missing or doubled newlines regardless of whether `text`
 * already ends in `\n`. Kept internal — the public surface is the two helpers.
 */
 ...
 */
export function parseError(message, usageText) {
    writeLine(process.stderr, message);
    if (usageText !== undefined) {
        writeLine(process.stderr, usageText);
    }
    return EXIT_USAGE;
}
/**
 ...
 */
export function declined(message) {
